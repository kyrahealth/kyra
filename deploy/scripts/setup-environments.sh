...
sudo cp deploy/nginx/kyra-production.conf /etc/nginx/sites-available/
...
# Enable sites
sudo ln -sf /etc/nginx/sites-available/kyra-production.conf /etc/nginx/sites-enabled/
...
echo ""
echo "📋 Next steps:"
echo "1. Add DNS records for demo.kyrahealth.ai, staging.kyrahealth.ai and dev.kyrahealth.ai"
echo "2. Get SSL certificates: sudo certbot --nginx -d demo.kyrahealth.ai -d staging.kyrahealth.ai -d dev.kyrahealth.ai"
...
echo "   Production: https://demo.kyrahealth.ai"
